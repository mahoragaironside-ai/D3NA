...
import { secaoSobre, secaoCatalogo, secaoTestemunhos } from "./secoes.js";

export function gerarEstrutura5(dados) {
  const {
    company_name = "O Meu Negócio", company_description = "",
    business_type = "", color_scheme = "azul", style_choice = 1,
    logo_choice = 1, contact_links = [], catalog_items = [],
  } = dados;
  ...
  const corpo = secaoSobre(company_description) + secaoCatalogo(catalog_items) + secaoTestemunhos();
